...
الترحيل. التكامل عبر IWorkflowPort (`modules.workflow.application.ports`)
فقط — لا استيراد لأي شيء من `workflow/infrastructure` هنا. التسلسل: إنشاء
الفاتورة ينشر `SalesInvoiceCreated` ← `StartManagerApprovalForSalesInvoiceUseCase`
(معالج داخلي لهذه الوحدة) يبدأ نسخة موافقة إن تجاوز المبلغ الحد ← `PostSalesInvoiceUseCase`
يرفض الترحيل ما دامت النسخة `pending_approval` أو `rejected`. الاعتماد
اختياري (`workflow_port: IWorkflowPort | None`) عمداً لتفادي كسر أي مسار
قائم لا يُمرِّر Adapter حقيقي صراحة — راجع `_NullWorkflowPort` أدناه.
"""
...
class _NullWorkflowPort:
    ...
    خارج نطاق ملفات مهمة #12، راجع بطاقة التسليم). لا يُنشئ أي نسخة موافقة
    أبداً، ويعتبر أي فاتورة "لا اعتراض" (`None`) — أي **لا تغيير** في سلوك
    أي مسار لا يُحقَن فيه Adapter حقيقي صراحة."""
...
class CreateSalesInvoiceUseCase:
    ...
    async def execute(self, ctx: TenantContext, request: SalesInvoiceCreateRequest) -> SalesInvoice:
        ...
        # sales/infrastructure/event_handlers.py (مهمة #12).
...
class PostSalesInvoiceUseCase:
    ...
    def __init__(
        ...
        # اختياري عمداً (راجع _NullWorkflowPort أعلاه) — كل مسار قائم لا
        # يُمرِّر Adapter حقيقي صراحة يستمر بالسلوك السابق تماماً دون تغيير
        # (مثال: modules/pos، خارج نطاق ملفات مهمة #12).
    ...
    async def execute(self, ctx: TenantContext, invoice_id: str) -> SalesInvoice:
        ...
        # بوابة موافقة المدير (مهمة #12): تُتحقَّق دائماً، بغض النظر عن قيمة
