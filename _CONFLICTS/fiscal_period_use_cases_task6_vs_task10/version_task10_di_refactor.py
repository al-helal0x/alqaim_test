...
مضاعف مع منع الترحيل المباشر). أخيراً يُنشر حدث `FiscalPeriodClosed` عبر
event_bus (contracts.md §2) حتى تُبلَّغ بقية الوحدات فوراً.
...
class ClosePeriodUseCase:
    ...
    async def _post_closing_entry(self, *, company_id: str, period: FiscalPeriod) -> str | None:
        ...
        assert_balanced(
            [JournalLineAmounts(debit=l["debit"], credit=l["credit"]) for l in pending_lines]
        )
