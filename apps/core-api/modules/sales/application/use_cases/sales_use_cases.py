...
from modules.sales.domain.rules import (
    SALES_INVOICE_APPROVAL_DEFINITION_NAME,
    SALES_INVOICE_APPROVAL_ENTITY_TYPE,
    SALES_INVOICE_APPROVAL_STATES,
    SALES_INVOICE_APPROVAL_TRANSITIONS,
    SALES_INVOICE_MANAGER_APPROVAL_THRESHOLD,
    LineInput,
    QuotationStatus,
    SalesInvoiceStatus,
    SalesOrderStatus,
...
class _NullWorkflowPort:
    """تنفيذ افتراضي "لا شيء" لـ IWorkflowPort — يُستخدَم فقط حين لا يُمرَّر
    `workflow_port` صراحة إلى `PostSalesInvoiceUseCase` (مثال: مسارات لا
    تُمرِّر بوابة موافقة بعد، كمسار POS checkout الحالي في `modules/pos` —
    خارج نطاق ملفات مهمة #12). لا يُنشئ أي نسخة موافقة أبداً، ويعتبر أي
    فاتورة "لا اعتراض" (`None`) — أي **لا تغيير** في سلوك أي مسار لا
    يُحقَن فيه Adapter حقيقي صراحة."""

    async def ensure_approval_started(self, request: ApprovalWorkflowRequest):
        return None
...
class InvoiceApprovalPendingError(ValueError):
    """فاتورة تتجاوز حد الموافقة (مهمة #12) ولم يوافق عليها مدير بعد."""


class InvoiceApprovalRejectedError(ValueError):
    """طلب موافقة المدير على هذه الفاتورة مرفوض — لا يمكن ترحيلها إطلاقاً
    بحالتها الحالية (يحتاج تعديل الفاتورة وإعادة تقديمها، خارج نطاق هذه
    المهمة)."""
...
def _to_line_inputs(lines: list[LineItemRequest]) -> list[LineInput]:
    return [
        LineInput(quantity=ln.quantity, unit_price=ln.unit_price, tax_amount=ln.tax_amount)
        for ln in lines
    ]
...
class CreateSalesInvoiceUseCase:
    ...
    async def execute(self, ctx: TenantContext, request: SalesInvoiceCreateRequest) -> SalesInvoice:
        # مهمة #11 — idempotency على مستوى الطلب: إن أُرسل مفتاح وسبق استخدامه
        # لهذه الشركة، أعد نفس الفاتورة المُنشأة أول مرة بدل إنشاء ازدواج. هذا
        # يغطي الحالة الشائعة (لا سباق حقيقي) بأقل تكلفة قبل أي تحقق/عمل إضافي.
        if request.idempotency_key:
            existing = await _find_invoice_by_idempotency_key(
                self._session, ctx.company_id, request.idempotency_key
            )
        ...
        # حدث حقيقة عمل عام (وليس خاصاً بالموافقة) — أي موديول آخر مستقبلاً
        # (تقارير/إشعارات) قد يهتم بـ "فاتورة بيع أُنشئت". القرار المتعلق
        # بموافقة المدير (>10,000) مسؤولية المشترك، وليست الناشر — راجع
        # StartManagerApprovalForSalesInvoiceUseCase أدناه ومعالجه في
        # sales/infrastructure/event_handlers.py (مهمة #12). مهمة #6: يُكتَب
        # ضمن نفس معاملة إنشاء الفاتورة قبل الـ commit — إما يُحفَظ الاثنان
        # معاً أو لا شيء (لو حدث IntegrityError أدناه، الـ rollback يُلغي
        # سطر outbox_events هذا أيضاً — لا حدث يتيم بلا فاتورة مقابلة). مسار
        # idempotent hit أعلى الدالة يعود مبكراً قبل الوصول هنا، فلا يُعاد
        # نشر الحدث لطلب مكرر (مهمة #11).
        await enqueue_event(
            self._session,
            event_name="SalesInvoiceCreated",
            payload={
        ...
        try:
            ...
            # سباق حقيقي: طلبان بنفس idempotency_key وصلا شبه-متزامنين وكلاهما
            # اجتاز التحقق أعلاه قبل أن يُنشئ أي منهما السجل — قيد UNIQUE على
            # مستوى القاعدة هو خط الدفاع الحاسم، وهذا هو مساره: تراجع وأعد نفس
            # الفاتورة التي فازت بالسباق بدل كسر الطلب الخاسر بخطأ للمستخدم.
...
class StartManagerApprovalForSalesInvoiceUseCase:
    """مشترك حدث `SalesInvoiceCreated` الداخلي لوحدة sales نفسها (مهمة #12):
    إن تجاوز إجمالي الفاتورة `SALES_INVOICE_MANAGER_APPROVAL_THRESHOLD` يبدأ
    نسخة موافقة على محرك workflow العام عبر `IWorkflowPort` — لا معرفة هنا
    بجداول workflow الداخلية إطلاقاً، فقط الواجهة المعلنة.

    يُستدعى فعلياً من `sales/infrastructure/event_handlers.py` (معالج خفيف
    مُسجَّل في `main.py` عند `event_bus.subscribe("SalesInvoiceCreated", ...)`
    — نفس نمط `PaymentRecorded`/`apply_payment_to_invoice` الموجود مسبقاً).

    ⚠️ التوقيع هنا (`__init__(workflow_port)` بلا session، وميثود `execute`
    وليس `handle`) مُطابِق **حرفياً** لِما كان مُنتظَراً مسبقاً في
    `modules/sales/infrastructure/event_handlers.py` — وليس تصميماً جديداً
    هنا."""

    def __init__(self, workflow_port: IWorkflowPort) -> None:
    ...
    async def execute(self, payload: dict) -> None:
        ...
        if not invoice_id or not company_id or total_amount is None:
            return  # حمولة غير مكتملة — تُهمَل بصمت (نفس نمط المعالجات الأخرى في main.py)
        ...
        if not requires_manager_approval(total):
            return  # ≤ 10,000 — لا تحتاج موافقة، لا تُنشأ أي نسخة workflow

        await self._workflow_port.ensure_approval_started(
            ApprovalWorkflowRequest(
                company_id=str(company_id),
                entity_type=SALES_INVOICE_APPROVAL_ENTITY_TYPE,
                entity_id=str(invoice_id),
                definition_name=SALES_INVOICE_APPROVAL_DEFINITION_NAME,
                states=SALES_INVOICE_APPROVAL_STATES,
                transitions=SALES_INVOICE_APPROVAL_TRANSITIONS,
            )
        )
...
class PostSalesInvoiceUseCase:
    ...
    def __init__(
        ...
        workflow_port: IWorkflowPort | None = None,
    ) -> None:
        self._session = session
        self._inventory_port = inventory_port
        self._accounting_port = accounting_port
        # مهمة #12: اختياري عمداً (راجع تعليق _NullWorkflowPort أعلاه) — أي
        # مسار قائم لا يُمرِّر Adapter حقيقي صراحة يبقى بلا فحص موافقة، تماماً
        # كسلوكه قبل هذا الدمج (مثال: modules/pos).
        self._workflow_port = workflow_port or _NullWorkflowPort()
    ...
    async def execute(self, ctx: TenantContext, invoice_id: str) -> SalesInvoice:
        ...
        # مهمة #11 — إعادة محاولة آمنة (idempotent) على مستوى الطلب: لو كانت
        # الفاتورة مُرحَّلة فعلاً (الترحيل السابق نجح لكن الرد ضاع على العميل،
        # timeout مثلاً)، أعد نفس النتيجة الناجحة فوراً بلا أي إعادة تنفيذ لأي
        # أثر جانبي (حجز/محاسبة/خصم) — هذا ما يجعل إعادة نفس طلب /post آمنة.
        ...
        # مهمة #12: بوابة موافقة المدير — تُتحقَّق دائماً، بغض النظر عن قيمة
        # المبلغ (إن لم تُبدَأ أي نسخة موافقة أصلاً، الحالة الشائعة: مبلغ
        # ≤ 10,000، فـ get_latest_instance_state تُعيد None ولا تأثير).
        approval_state = await self._workflow_port.get_latest_instance_state(
            company_id=ctx.company_id,
            entity_type=SALES_INVOICE_APPROVAL_ENTITY_TYPE,
            entity_id=str(invoice.id),
        )
        if approval_state == "pending_approval":
            raise InvoiceApprovalPendingError(
                f"الفاتورة {invoice.invoice_number} تتجاوز حد "
                f"{SALES_INVOICE_MANAGER_APPROVAL_THRESHOLD:,} وبانتظار موافقة مدير قبل الترحيل"
            )
        if approval_state == "rejected":
            raise InvoiceApprovalRejectedError(
                f"طلب الموافقة على الفاتورة {invoice.invoice_number} مرفوض — لا يمكن ترحيلها"
            )
        ...
        # مهمة #11 — "نجح الإنشاء وفشل الترحيل": هذا بالضبط المثال الموثَّق أعلى
        # الملف (فترة مالية مقفلة مثلاً). قبل هذا الإصلاح كانت الحجوزات تبقى
        # معلَّقة حتى تنتهي صلاحيتها تلقائياً (15 دقيقة) — الآن تُحرَّر فوراً
        # حتى تنجح إعادة محاولة فورية لنفس invoice_id (draft) بلا أي أثر على
        # المخزون وبلا الحاجة لانتظار انتهاء الصلاحية، وبلا أي ازدواج للفاتورة
        # نفسها (لم تُنشأ فاتورة جديدة، فقط أُعيد ترحيل نفس invoice_id).
        try:
        ...
        for product_id, reservation_id in reservations:
            line = next(ln for ln in invoice.lines if str(ln.product_id) == product_id)
        ...
        # مهمة #6 (Outbox): يُكتَب ضمن نفس معاملة الترحيل قبل الـ commit —
        # إما يُحفَظ ترحيل الفاتورة + الحدث معاً ذرّياً أو لا شيء منهما.
        await enqueue_event(
            self._session,
            event_name="InvoicePosted",
            payload={
                "invoice_id": str(invoice.id),
                "invoice_type": "sale",
...
async def _find_invoice_by_idempotency_key(
    session: AsyncSession, company_id: str, idempotency_key: str
) -> SalesInvoice | None:
    stmt = (
        select(SalesInvoice)
        .where(SalesInvoice.company_id == company_id, SalesInvoice.idempotency_key == idempotency_key)
        .options(selectinload(SalesInvoice.lines))
    )
