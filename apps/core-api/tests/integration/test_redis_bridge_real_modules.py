...
async def _redis_available() -> bool:
    try:
        ...
        await client.aclose()
        return True
    except Exception:  # noqa: BLE001 — فحص توفّر Redis فقط: أي فشل هنا (اتصال/DNS/نسخة) يعني ببساطة "تخطَّ الاختبار", لا داعٍ لتمييز نوع الاستثناء
        return False
...
async def test_real_bridge_receives_event_from_separate_ai_platform_process():
    ...
    try:
        ...
        result = subprocess.run(  # noqa: ASYNC221 — عمداً متزامن: يشغّل عملية Python منفصلة (محاكاة ai-platform) وينتظر خروجها كجزء من سيناريو E2E، ليس استدعاءً محظوراً في مسار إنتاج غير متزامن
