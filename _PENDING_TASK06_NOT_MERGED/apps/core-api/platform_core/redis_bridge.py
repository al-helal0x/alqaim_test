...
async def _dispatch_envelope(raw_message: bytes | str) -> None:
    ...
    try:
        # لا معاملة أعمال قائمة هنا (الحدث قادم من عملية خارجية عبر Redis، لا
        # من use case محلي) — نفتح جلسة مخصَّصة فقط لكتابة outbox_events ثم
        # نحاول التسليم فوراً؛ أي فشل مؤقت في مشترك يلتقطه Worker الدوري لاحقاً
        # (platform_core/outbox_worker.py) دون فقدان الحدث.
